...
namespace Factory 
{
	...
	{
		std::shared_ptr<antibox::Mesh> mesh = std::make_shared<antibox::Mesh>(&Vertices[0], 4, 3, &texcoords[0], &Elements[0], 6);
		std::shared_ptr<antibox::Shader> shader = std::make_shared<antibox::Shader>(DefaultVert, DefaultFrag);

		glm::vec2 size{ 0.5,0.5 };
		auto ret = std::make_shared<antibox::GameObject>(mesh, shader, pos, size, texture);
		return ret;
	}*/
}